...
library(dfcrm)  #function crmht getprior
...
find_param <- function(p, tstar, hazard){
  ...
  } else {param <- (- log(1-p)/tstar)}#decreasing time-to-toxicity hazard
  return(param)
}
...
getdataset_CR <- function(n, scenario1, scenario2, tstar, seed) {
  set.seed(seed)
  K <- length(scenario)
  Dataset <- cbind(Patient=rep(1:n, each=K),Ui= rep(runif(n), each=K),Time=rep(NA,n*K),Event=rep(NA,n*K),Cens_time=rep(NA,n*K),Dose=rep(1:K,n),I_Tox=rep(NA,n*K) )  
  i = 1
  while(i<=n){
...
isurvcrm_nextdose <- function(event, dose_level, xref1,xref2,time,tstar,target){
  ...
  beta_hat1 <-   integrate(num_tox, -Inf, Inf,  event=event, dose_level=dose_level, xref=xref1,time=time,abs.tol = 0)$value / constante1
  ...
  lambda1 <- exp(exp(beta_hat1)*xref1)           
  ...
  #Next dose finding algorithm
  Diff_pestim_target <- abs(CI_temp1 - target)
  Doses_min <- which(Diff_pestim_target==min(Diff_pestim_target)  )
  next_dose <- ifelse(length(Doses_min)==1,Doses_min,sample(Doses_min,1)) 
...
#' 
survcrm_sim <- function(N,scenario,n,target,tstar,dose_init,rate,accrual,hazard,delta,nu){
  K <- length(scenario)
  ...
  skeleton <- getprior_exp(halfwidth=delta, target, nu=nu, nlevel=K, tstar=tstar) 
...
isurvcrm_sim <- function(N,scenario1,scenario2,n,target,tstar,dose_init,rate,accrual,skeleton1,skeleton2){
  ...
  #Set of numerical labels for the doses investigated in the trial
  ...
  while (s<=N) {
    #git
    Dataset <- getdataset_CR(n, scenario1,scenario2,  tstar, seed=s)  #Complete data generation for n patients
    if (accrual == "fixed") {                       #Patient accrual scheme
      next_arrival <- tstar/rate
    }   else if (accrual == "poisson") {
      next_arrival <- rexp(1, rate/tstar)
    ...
    while(i <=(n-1)){
      ...
      event <- rep(0, length(Dataset_temp[,"Event"])) #Vector of toxicity occurrence
    ...
    if(i==n){
      ...
      CI_estim1[s,] <- res$CI_temp1                    #Cumulative incidence estimate at t* for trial s 
      Dose_select[next_dose] <- Dose_select[next_dose]+1  
      No_MTD[s] <- sum(Dataset_temp[,"Dose"] %in% correct)
      OD[s] <- sum(Dataset_temp[,"Dose"] %in% toxic)
      No_DLT[s] <- sum(Dataset_temp[,"Event"])
  ...
  return(list(Scenario=round(scenario1,2),
              PS=round(PS,0),
              PCS = round(sum(PS[correct]),0),
...
titecrm_sim <- function(N,scenario,n,target,tstar,dose_init,rate,accrual,hazard){
  K <- length(scenario)
  diff <- as.numeric(as.character(abs(scenario-target))) 
  toxic <- which(scenario > unique(scenario[which(diff== min(diff))]))
