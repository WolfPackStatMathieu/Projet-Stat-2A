...
function_estim_doses_comp<-function(n,probabilite_a_priori,t_star,type1){
  ...
  for (k in c(1:nb_doses)){
    ...
    df[index_dosek,]<-cbind(rep(k,n_k),generation_comp(p_cause1=p,p_cause2=1-p,t_star,nombre_obs=n_k,type1,type2=type1))
...
test_mult_doses<-function_estim_doses_comp(n=25,probabilite_a_priori = prob_priori,t_star=6,type1 = "decreasing")
