...
test_evol_biais<-evol_biais_comp(K=1900,probabilite_a_priori=prob_priori,t_star=6,type1="constant",graine_depart=133)
...
evol_eqm_comp<-function(K,probabilite_a_priori,t_star,type1,graine_depart){
  ...
  print(results)
  ensemble_ggplots_par_dose<-lapply(c(1:length(probabilite_a_priori)),evol_n_par_dose_eqm,results=results,n=n,K=K, type1=type1)
  return(ensemble_ggplots_par_dose)
}
evol_n_par_dose_eqm<-function(results,n,i,K=K, type1){
  longueur_resultats<-c(1:length(n))
  function_intermed<-function(x,results,i){
    return(unlist(results[[x]][i,]))
  }
  ...
  gg1 <- {ggplot(data = result_final, aes(x = taille_echantillon)) +
      ...
      theme(legend.title=element_blank(),
            ...
            plot.title = element_text(family = "Helvetica", size = 10)) +
      ylim(borne_min, borne_max) +
      labs(caption = sprintf("p=%s, N=%s, type = %s",
                             as.character(result_final$p),
                             as.character(K)
                             ,type1))}
  gg2 <- {ggplot(data = result_final, aes(x = taille_echantillon)) +
...
# test<-evol_eqm_comp(K=15,probabilite_a_priori=prob_priori,t_star=6,type1="decreasing",graine_depart=145)
