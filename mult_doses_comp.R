...
function_estim_doses_comp<-function(n,probabilite_a_priori,t_star,type1,type2,graine=133){
  ...
  if(length(indice_cens)==0){
    ...
    Prob_whole_curefx<-result<-flexsurvcure(Surv(tox_time,is_observed)~factdose, data=df, dist="weibullPH",
                                            anc=list(scale=~factdose))
    Prob_whole_cure<-fit.cure.model(Surv(tox_time,is_observed) ~ factdose, data =df2,dist="weibull",link="logit")
    beta0<-as.numeric(Prob_whole_cure$coefs[1]$'1')
    deux_beta<-as.numeric(Prob_whole_cure$coefs[1]$'1'[c(2,nb_doses)])
    coeffs<-beta0+c(0,deux_beta)
...
test_mult_doses<-function_estim_doses_comp(n=100,probabilite_a_priori = prob_priori,t_star=6,type1 = "decreasing",graine=133,type2="decreasing")
